...
#update git version
#yum -y install perl-ExtUtils-MakeMaker tk zlib-devel openssl-devel perl cpio expat-devel gettext-devel 
yum -y remove git
yum -y install wget zlib-devel openssl-devel cpio expat-devel gettext-devel curl-devel perl-ExtUtils-CBuilder perl-ExtUtils-MakeMaker tk
cd /tmp
...
# GitLab ShellYour user account isn't allowed to install to the system Rubygems.

## Clone gitlab-shell
su - git -c "git clone https://github.com/gitlabhq/gitlab-shell.git"
su - git -c "cd gitlab-shell;git checkout v1.9.6"
...
su git -c "bundle --without aws development test mysql --deployment"
...
su git -c "bundle exec rake cache:clear RAILS_ENV=production"
...
yum -y install nginx
chkconfig nginx on
sudo curl --output /www/wdlinux/nginx/conf/vhost/gitlab	
cd /www/wdlinux/nginx/conf/vhost/
#mv default.conf default.conf.bak
mv gitlab teamserver.7766.org.conf
sed -i "s/unix:\/home\/git\/gitlab\/tmp\/sockets\/gitlab.socket/127.0.0.1:3000/g" /www/wdlinux/nginx/conf/vhost/teamserver.7766.org.conf
sed -i "s/YOUR_SERVER_FQDN/$GL_HOSTNAME/g" /www/wdlinux/nginx/conf/vhost/teamserver.7766.org.conf

sudo usermod -a -G git www
